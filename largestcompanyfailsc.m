...
% used to compare the functions w.r.t. fixed theta, sigma, and various c
...
d_range = [1:1/2:20];
std_range = [0:0.3:0.2];
AvFailuresC = zeros(length(d_range), length(c_range));
...
AvFailures3d=zeros(length(d_range),length(c_range),length(theta_range));
AvSomeFailures3d=zeros(length(d_range),length(c_range),length(theta_range));
...
for c = c_range
    ...
    for std=std_range
        ...
        for theta=theta_range
            ...
            for d=d_range
			    ...
			    AvSomeFailures3d(d_counter,c_counter,theta_counter)=SomeFailures./iterations;
    ...
    AvFailuresC(:,c_counter) = AvFailures3d(:,c_counter);
end
%%
plot(AvFailuresC,'-o')
legend('c = 0.3', 'c = 0.5',  'c = 0.7', 'c = 0.9')
title('Average Failures over 100 Iterations with theta = 0.7, std = 2')
